...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    try {
        ...
        $pdo = new DB();
        // Preparar la consulta SQL para eliminar el usuario en la tabla "users".
        $pdo->query("DELETE FROM users WHERE userId = ?");
        ...
        if ($pdo->execute()) {
            ...
            $response = array(
                ...
                "text" => $_SESSION["lang"]["UsersControl.php"]["ThrowModal"]["UserDeleted"],
        ...
        $response = [
            ...
            "alertType" => "danger",
            "confirmMessage" => $_SESSION["lang"]["UsersControl.php"]["ThrowModal"]["Confirm"],
            "text" => $_SESSION["lang"]["UsersControl.php"]["ThrowModal"]["UserNotDeleted"],
            "buttonText" => $_SESSION["lang"]["UsersControl.php"]["Buttons"]["Close"],

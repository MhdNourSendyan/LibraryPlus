...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    try {
        ...
        $newBookIdentifier = $data->newBookIdentifier;
        $newBookName = $data->newBookName;
        $newBookAuthor = $data->newBookAuthor;
        $newBookEditorial = $data->newBookEditorial;
        $newBookQuantity = $data->newBookQuantity;
        // Crear una nueva conexión a la base de datos.
        $pdo = new DB();
        // Preparar la consulta SQL para insertar un nuevo libro en la tabla "books".
        $pdo->query("INSERT INTO books (bookIdentifier, bookName, bookAuthor, bookEditorial, bookQuantity) VALUES (?, ?, ?, ?, ?)");
        $pdo->bind(1, $newBookIdentifier, PDO::PARAM_STR);
        $pdo->bind(2, $newBookName, PDO::PARAM_STR);
        $pdo->bind(3, $newBookAuthor, PDO::PARAM_STR);
        $pdo->bind(4, $newBookEditorial, PDO::PARAM_STR);
        $pdo->bind(5, $newBookQuantity, PDO::PARAM_INT);
        ...
        if ($pdo->execute()) {
            ...
            $response = [
                ...
                "text" => $_SESSION["lang"]["Main.php"]["ThrowModal"]["BookAdded"],
        ...
        if ($e->getCode() == 23000 && strpos($e->getMessage(), "bookIdentifier") !== false) {
            ...
            $response = [
                ...
                "alertType" => "danger",
                "confirmMessage" => $_SESSION["lang"]["Main.php"]["ThrowModal"]["Confirm"],
                "text" => $_SESSION["lang"]["Main.php"]["ThrowModal"]["DuplicateBookIdentifier"],
                "buttonText" => $_SESSION["lang"]["Main.php"]["Buttons"]["Close"],
            ...
            $response = [
                ...
                "alertType" => "danger",
                "confirmMessage" => $_SESSION["lang"]["Main.php"]["ThrowModal"]["Confirm"],
                "text" => $_SESSION["lang"]["Main.php"]["ThrowModal"]["BookNotAdded"],
                "buttonText" => $_SESSION["lang"]["Main.php"]["Buttons"]["Close"],
                "exceptionMessage" => $e->getMessage(),
            ];
        }

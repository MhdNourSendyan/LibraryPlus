...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    try {
        ...
        $pdo = new DB();
        // Preparar la consulta SQL para eliminar el dispositivo en la tabla "devices".
        $pdo->query("DELETE FROM devices WHERE deviceId = ?");
        $pdo->bind(1, $hiddenDeviceId, PDO::PARAM_INT);
        ...
        if ($pdo->execute()) {
            ...
            $response = [
                ...
                "text" => $_SESSION["lang"]["Main.php"]["ThrowModal"]["DeviceDeleted"],
        ...
        $response = [
            ...
            "alertType" => "danger",
            "confirmMessage" => $_SESSION["lang"]["Main.php"]["ThrowModal"]["Confirm"],
            "text" => $_SESSION["lang"]["Main.php"]["ThrowModal"]["DeviceNotDeleted"],
            "buttonText" => $_SESSION["lang"]["Main.php"]["Buttons"]["Close"],

...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    try {
        ...
        $userId = $data->userId;
        $messageDemand = $data->messageDemand;
        $messageText = $data->messageText;
        // Crear una nueva conexión a la base de datos.
        $pdo = new DB();
        // Preparar la consulta SQL para editar el dispositivo en la tabla "messages".
        $pdo->query("INSERT INTO messages (userId, messageDemand, messageText) VALUES (?, ?, ?)");
        $pdo->bind(1, $userId, PDO::PARAM_INT);
        $pdo->bind(2, $messageDemand, PDO::PARAM_STR);
        $pdo->bind(3, $messageText, PDO::PARAM_STR);
        ...
        if ($pdo->execute()) {
            ...
            $response = [
                "success" => true,
                "alertType" => "success",
                "confirmMessage" => $_SESSION["lang"]["sendMessages.php"]["ThrowModal"]["Confirm"],
                "text" => $_SESSION["lang"]["sendMessages.php"]["ThrowModal"]["MessageSent"],
                "buttonText" => $_SESSION["lang"]["sendMessages.php"]["Buttons"]["Close"],
            ];
        ...
        $response = [
            ...
            "alertType" => "danger",
            "confirmMessage" => $_SESSION["lang"]["sendMessages.php"]["ThrowModal"]["Confirm"],
            "text" => $_SESSION["lang"]["sendMessages.php"]["ThrowModal"]["MessageNotSent"],
            "buttonText" => $_SESSION["lang"]["sendMessages.php"]["Buttons"]["Close"],
